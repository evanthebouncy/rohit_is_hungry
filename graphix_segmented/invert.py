...
class Inverter:
  ...
  def invert_cegis(self, constraints, r_squares, r_lines, method="cegis"):
    full_img = np.clip(r_squares + r_lines, 0, 1)
    assert method in ["cegis", "r_cegis", "nn_cegis", "nn",]
    i = 0
    sub_constraints = constraints[:2]
    ...
    while True:
      ...
      i += 1
      paras = self.s.solve(15, sub_constraints, {})
      print paras
  ...
  def invert_full(self, constraints, r_squares, r_lines, method="full"):
    full_img = np.clip(r_squares + r_lines, 0, 1)
    assert method in ["full", "rand", "nn"]

    if method == "full":
      params = self.s.solve(15, constraints)
      return params

    if method == "rand":
      sub_constraints = [c for c in constraints if random.random() < 0.9]
      params = self.s.solve(15, sub_constraints)
      return params

    if method == "nn":
      trace_obs = self.impnet.get_trace(full_img)
